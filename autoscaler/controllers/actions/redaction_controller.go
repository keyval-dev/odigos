...
var redactionSupportedSignals = map[common.ObservabilitySignal]struct{}{
	common.TracesObservabilitySignal: {},
}

type RedactionReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

func (r *RedactionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx)
	logger.V(0).Info("Reconciling AddClusterInfo action")

	action := &actionv1.Redaction{}
	...
	for _, signal := range action.Spec.Signals {
		...
		if _, ok := redactionSupportedSignals[signal]; !ok {
...
func (r *RedactionReconciler) ReportReconciledToProcessorFailed(ctx context.Context, action *actionv1.Redaction, reason string, msg string) error {
...
func (r *RedactionReconciler) ReportReconciledToProcessor(ctx context.Context, action *actionv1.Redaction) error {
	changed := meta.SetStatusCondition(&action.Status.Conditions, metav1.Condition{
		Type:               ActionTransformedToProcessorType,
...
func (r *RedactionReconciler) convertToProcessor(action *actionv1.Redaction) (*v1.Processor, error) {

	redactionAttributes := action.Spec.RedactionAttributes
	if !redactionFieldsExist(redactionAttributes) {
		return nil, fmt.Errorf("none of the attributes are configured to be redact, no need for this processor")
	}

	// Allow all attributes to be traced. If configured to false it redact all the attributes
	config := RedactionConfig{
		AllowAllKeys: true,
	}

	for _, attr := range redactionAttributes {
		switch attr.RedactAttributeName {
		case actionv1.CreditCardRedaction:
...
func redactionFieldsExist(attributes []actionv1.RedactionAttribute) bool {
